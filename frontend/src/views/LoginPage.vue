<template>
  <div class="auth-page">
    ...
    <div class="auth-box card-glass">
      ...
      <p class="auth-sub">Ingresa a tu plataforma de investigación</p>

      <!-- Social buttons -->
      <div class="social-btns">
        <button class="btn btn-ghost social-btn" @click="loginGoogle">
        ...
        <button class="btn btn-ghost social-btn" @click="loginGithub">
      ...
      <div class="or-divider"><span>o con email</span></div>
      ...
      <p class="auth-footer">
        ¿No tienes cuenta? <RouterLink to="/register">Créala gratis</RouterLink>
      </p>
    </div>
  </div>
</template>
...
async function handleLogin() {
  ...
  if (ok) router.push((route.query.redirect as string) || '/app')
  else { error.value = 'Credenciales incorrectas. Intenta de nuevo.'; loading.value = false }
}
async function loginGoogle() {
  const { url } = await auth.getGoogleUrl()
  window.location.href = url
}
async function loginGithub() {
  const { url } = await auth.getGithubUrl()
  window.location.href = url
}
</script>
...
.social-btn { justify-content: center; gap: 10px; font-size: 14px; }
.or-divider {
  display: flex; align-items: center; gap: 12px;
  margin: 24px 0; color: var(--gray-600); font-size: 12px;
}
